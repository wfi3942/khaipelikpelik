								...
								},								
	    	    		        {
		    	    				...
		    	    				render: function(data, type, row, meta) {
		    	    				    if (row.project_status == 0) {
		    	    				    	...
		    	    				    	//按钮变成可选状态
		    	    				        var run = '<button type="button" name="btn-project-run" value="'+ row.id +'" class="btn btn-info" aria-label="Justify"><a href="/cicd/manage/?type=status&id='+ row.id + '"><span class="fa fa-play" aria-hidden="true"></span></a>' + '</button>'
		    	    				    }
		    	    					if (row.project_repertory=='git' && row.project_model == 'branch'){
		    	    						//按钮可选状态
		    	    						var branch = '<button type="button" name="btn-project-branch" value="'+ row.id +'" class="btn btn-warning"  aria-label="Justify"><span class="fa fa-github-alt" aria-hidden="true"></span></button>'
		    	    					} else{
		    	    						//按钮不可选状态
		    	                        ...
		    	                        return '<div class="btn-group  btn-group-sm">' +
			    	                           run +
			    	                           '<button type="button" name="btn-project-edit" value="'+ row.id +'" class="btn btn-primary"  aria-label="Justify"><a href="/cicd/config/?type=edit&id='+ row.id +'"><span class="fa fa-pencil-square-o" aria-hidden="true"></span></a>' + '</button>' +
			    	                           branch +
			    	                           '<button type="button" name="btn-project-delete" value="'+ row.id +'" class="btn btn-danger" aria-label="Justify"><span class="fa fa-trash" aria-hidden="true"></span>' + '</button>' +
			    	                           '</div>';
		    	    				},
		    	    				"className": "text-center",
	    	    		        },
	    	    		      ]
		    return InitDataTable('projectList','/api/config_list/',columns,columnDefs);
		    //每隔30秒刷新table
        		    ...
        		    buttons: {
        		        确认:
        		        	...
        		        	$.ajax({  
        		                ...
        		                async: true,  
        		                error: function(response) {  
        		                	swal('Ops Error', 'Ops Error' + "[ " + response.responseText + " ]" + 'Error', "error");
        		                },  
        		                success: function(response) {  
        		                	if (response["code"] == 200){
        		                    	swal({title: 'Success!', text:  response["msg"], type: "success"}, function () {
                                			RefreshTable('#projectList', '/api/config_list/');
                            			})
        		                	}else{
        		                    	swal('Ops Error', 'Ops Error' + "[ " + response["msg"] + " ]" + 'Error', "error");
        		                	}
        		                }  
        		        	});          		        	
        		        },
