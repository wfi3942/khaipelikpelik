		...
		success:function(response){
			...
			for (var i=0; i <response.length; i++){
				if (ids==response[i]["id"]){
					selectHtml += '<option selected="selected" value="'+ response[i]["id"] +'">'+ response[i]["name"] +'</option>'
				}else{
...
$(document).ready(function() {
	...
	url = '/cicd/config/?type=info&id=' + get_url_param('id')
    ...
    $('#btn_modf_project').click(function(){
    	swal({
                ...
                closeOnConfirm: false
					...
					$.ajax({
						url: '/cicd/config/?type=edit', //请求地址
						type: "POST",  //提交类似
						processData: false,
						contentType: false,
						data: formData,  //提交参数
						success: function (response) {
							...
							if (response["code"] == 200) {
								swal({title: 'Update', text:  'Update Successfully', type: "success"},function () {
									//跳转到发布列表页
